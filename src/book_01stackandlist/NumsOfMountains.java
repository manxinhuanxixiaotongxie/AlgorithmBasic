 ...
 * 比如{3,1,2,4,5} {4,5,3,1,2}，{1,2,4,5,3}都代表相同结构的环形山
 * 3->1-2-4-5-3方向叫做next方向(逆时针)
 * 3->5-4-2-1-3方向叫做prev方向(顺时针)
 * 山峰A和山峰B能够相互看见的条件为:
 * 1.如果A和B是同一座山，认为不能相互看见。
 * 2.如果A和B是不同的山，并且在环中相邻，认为可以相互看见。比如图1-8中，相邻的山峰对有(1,2)(2,4)(4,5)(3,5)(1,3)。
 * 3.如果A和B是不同的山，并且在环中不相邻，假设两座山高度的最小值为min。如果A通过 next 方向到B的途中没有高度比min 大的山峰，
 * 或者A通过 last 方向到B的途中没有高度比 min 大的山峰，认为A和B可以相互看见。比如图1-8中，高度为3的山和高度为4的山，
 * 两座山的高度最小值为3。3从last方向走向4，中途会遇见5，所以last方向走不通;3从 next方向走向4,中途会遇见1和2,但是都不大于两座山高度的最小值3,所以 next方向可以走通
 *
 * 给定一个不含有负数且没有重复值的数组arr 返回有多少对山峰能够互相看见
 * 要求：
 * 如果数组arr的长度为N 没有重复值情况下的时间复杂度为O（1） 可能有重复值情况下时间复杂度请达到O(N)
 */
public class NumsOfMountains {
